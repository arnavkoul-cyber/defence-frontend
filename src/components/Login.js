    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="relative flex flex-col min-h-screen">
        ...
        <div className="flex flex-1 items-center justify-center p-4">
          <div className="w-full max-w-5xl bg-white/80 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 overflow-hidden flex flex-col md:flex-row">
            {/* Left image side */}
            <div className="relative hidden md:block md:w-1/2">
              <img src={require('../assets/labour_pic.jpg')} alt="Labour" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/20" aria-hidden="true"></div>
              {/* Headline centered */}
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
                <h3 className="text-white text-xl font-semibold tracking-wide">DEFENCE LABOUR LOGIN PORTAL</h3>
                <p className="text-white/90 mt-3"></p>
              </div>
            </div>

            {/* Right form side */}
            <div className="w-full md:w-1/2 p-8">
              <h2 className="text-3xl font-extrabold mb-6 text-center text-blue-50 md:text-blue-800 tracking-wide drop-shadow">Officer Login</h2>
              ...
              <div className="mb-4">
                <div className="relative">
                  <button
                    type="button"
                    className="w-full p-3 border border-white/40 md:border-gray-300 rounded-lg bg-white/80 md:bg-white text-left flex items-center justify-between focus:outline-none focus:ring-2 focus:ring-green-400"
                    onClick={() => setIsUserTypeOpen((o) => !o)}
                    onBlur={() => setTimeout(() => setIsUserTypeOpen(false), 150)}
                  >
                    <span className={`${userType ? 'text-gray-900' : 'text-gray-500'}`}>{userType || 'Select user type'}</span>
              ...
              <input type="tel" aria-label="Mobile Number" placeholder={`Enter registered Mobile Number`} value={mobile} onChange={(e) => setMobile(e.target.value)} className="w-full p-3 mb-4 border border-white/40 md:border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-400 transition text-base shadow-sm bg-white/80 md:bg-white" />

              <div className="flex justify-center mb-4">
                <ReCAPTCHA sitekey={SITE_KEY} onChange={handleCaptcha} />
              </div>

              <button type="button" onClick={handleLogin} disabled={!isMobileValid(mobile) || !captchaVerified || loading} className={`w-full px-6 py-3 rounded-xl font-semibold shadow-md transition-all duration-300 ease-in-out ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-green-600 to-green-400 text-white hover:from-green-700 hover:to-green-500'}`}>
